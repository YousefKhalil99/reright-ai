...
async function getSuggestions(text) {
    // Always use offscreen document (via background script) to centralize AI logic
    try {
        const response = await chrome.runtime.sendMessage({
            type: 'getSuggestionsViaOffscreen',
            text: text
        });
        ...
        if (response && response.success) {
            ...
            throw new Error(response?.error || "Chrome built-in AI is not available. Please check the debug page.");
        }
    } catch (err) {
        throw new Error(err.message || "Chrome built-in AI is not available. Ensure you have enabled the required flags.");
    }
}
